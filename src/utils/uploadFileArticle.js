...
const UPLOAD_DIR = '/images/articles/';
 ...
 */
const getUploadFilepath = async (file, dir) => {
  // File validation
  ...
  return `${UPLOAD_DIR + dir}/${randomFilename}`;
};
 ...
 */
const uploadFileArticle = async (file, dir) => {
  if (!file) return null;
  ...
  const serverUploadDir = filesystem.upload.baseDir + UPLOAD_DIR + dir;
  ...
  if (!fs.existsSync(serverUploadDir)) {
    ...
    try {
      ...
      console.error('File Article Upload Error:', error);
      throw error;
    }
  }
  ...
  try {
    if (fileSize > filesystem.upload.markAsLargeFile) {
      ...
      writeStream.on('error', (err) => {
        ...
        console.error('File Article Upload Error:', err);
        deleteFile(filePath);
        throw err;
      });
    } else {
 ...
 */
const updateFileArticle = async (oldFilePath, newFilePath, data) => {
  try {
